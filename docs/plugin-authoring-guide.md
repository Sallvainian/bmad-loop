...
`post_story` fires after an isolated unit's worktree is torn down, so a declarative
`post_story` hook then runs from the repo root, and `BMAD_LOOP_WORKTREE` still names
