...
![CLIs](https://img.shields.io/badge/agents-claude%20%C2%B7%20codex%20%C2%B7%20gemini%20%C2%B7%20copilot%20%C2%B7%20antigravity%20%C2%B7%20grok%20%C2%B7%20opencode-8a2be2)
...
- ♻️ **Resumable & multi-agent.** Every run is a resumable state machine on disk, and a generic tmux adapter drives `claude`, `codex`, `gemini`, `copilot`, `antigravity`, or `grok` (mix per stage).
...
| `bmad-loop delete <run-id>`                              | Delete a run directory. `--force` stops the run first if it is still live, but cannot override a rival resume that becomes live before removal. Unknown liveness still warns and proceeds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `bmad-loop archive <run-id>`                             | Compress a run into `.bmad-loop/archive` and remove the run dir. `--force` stops the run first if it is still live, but cannot override a rival resume that becomes live before archival. Unknown liveness still warns and proceeds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
```text
bmad-loop sweep [--only DW-1,DW-3 | --min-severity LEVEL] [--no-prompt] [--decisions-only] [--max-bundles N] [--repeat] [--max-cycles N] [--dry-run]
  │
  ├─ TRIAGE   fresh window: claude "/bmad-loop-sweep"
  │           verifies every selected open entry against the actual code (all
  │           open entries when no selector is given; ledger
...
`--only` accepts an exact, comma-separated set of currently open `DW-<n>` ids;
unknown or non-open ids fail the initial run instead of silently narrowing it.
`--min-severity low|medium|high|critical` includes the named level and higher;
entries with missing or unrecognized severity are excluded and reported. Both
selectors apply before triage, bundle formation, and `--max-bundles`, appear in
dry-run output, persist across resume, and journal the excluded ids. A repeated
severity sweep re-evaluates the current ledger, while a repeated named sweep can
...
max_review_cycles = 3
max_dev_attempts = 2
# Artifact publication uses binary MiB: 1 MiB = 1,048,576 raw bytes.
artifact_file_max_mb = 5          # per ignored artifact selected for isolated publication
artifact_payload_max_mb = 10      # aggregate before base64 state encoding
...
[adapter]
name = "claude"            # CLI profile: claude | codex | gemini | copilot | antigravity | grok | opencode-http (alias: opencode) | custom
...
- **`artifact_file_max_mb` / `artifact_payload_max_mb`** (defaults 5 / 10) — inclusive per-file and aggregate raw-byte admission limits for ignored artifacts published from an isolated bundle. Values are binary MiB (`1 MiB = 1,048,576 bytes`); tracked declarations ride Git and do not count.
  Raising these limits also raises persisted state and transient memory use: base64 alone expands admitted bytes by roughly one third, and the payload is copied on each state save.
...
| `grok`        | experimental                             | xAI **Grok CLI** (`grok` ≥ 1.0.30). Hooks use the Claude settings shape in `.grok/hooks/bmad-loop.json`; skills live in `.agents/skills/`, where the BMad installer puts them; `--always-approve` for unattended runs. Each subagent is its own grok session and fires `SessionEnd` on teardown, so the profile sets `ignore_foreign_session_end` and a subagent finishing is not scored as a crash. Project hooks run only after `/hooks-trust` in the project; trust covers subdirectories, so worktree isolation works. `grok-updates` sums each turn's usage, including subagents'.                                                                                                                                                                                                                                                                                                    |
