...
| `grok`        | experimental                             | xAI **Grok CLI** (`grok` ≥ 1.0.41). Hooks use the Claude settings shape in `.grok/hooks/bmad-loop.json`; skills live in `.agents/skills/`, where the BMad installer puts them; `--always-approve` for unattended runs. Each subagent is its own grok session and fires `SessionEnd` on teardown, marked with `subagentType`, so a subagent finishing is not scored as a crash. Project hooks run only after `/hooks-trust` in the project; trust covers subdirectories, so worktree isolation works. `grok-updates` sums each turn's usage, including subagents'.                                                                                                                                                                                                                                                                                                                          |
